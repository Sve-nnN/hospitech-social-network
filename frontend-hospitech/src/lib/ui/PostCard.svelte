...
<Card.Root class="mb-4">
	<Card.Header class="p-4 pb-0">
		<div class="flex items-center gap-3">
			...
			<div class="flex-1">
				<a
					href={`/profile/${post.user_info?.username}`}
	...
	<Card.Content class="p-4">
		...
		{#if post.imagenes_url && post.imagenes_url.length > 0}
			<div class="w-full">
				<img
					src={post.imagenes_url[0]}
					alt="Post"
					class="w-full max-h-[500px] object-cover rounded-md"
				/>
			</div>
		{/if}
	</Card.Content>
	...
	<Card.Footer class="flex flex-col p-0">
		<div
			class="px-4 py-2 flex items-center justify-between text-sm text-muted-foreground border-t w-full"
		>
				...
				{#each comments as comment}
					<div class="flex gap-2">
						<Avatar.Root class="h-8 w-8">
							...
							<Avatar.Fallback
								>{comment.user_info?.username?.[0]?.toUpperCase() || 'U'}</Avatar.Fallback
							>
						</Avatar.Root>
