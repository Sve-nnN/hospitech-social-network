<script>
	...
	function verDetalle(idOrSlug) {
		if (!idOrSlug) return;
		if (idOrSlug) {
			// Always navigate to /hotel/[id] since that's the only route we have
			goto(`/hotel/${idOrSlug}`);
		}
	}
</script>
	...
	{:else}
		<div class="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
			{#each hoteles as hotel}
				<Card.Root
					class="group hover:shadow-lg transition-all duration-300 border-border/50 bg-card/50 backdrop-blur-sm overflow-hidden"
				>
					{#if hotel.imagenes_url && hotel.imagenes_url.length > 0}
						<div class="h-48 w-full overflow-hidden">
							<img
								src={hotel.imagenes_url[0]}
								alt={hotel.nombre}
					...
					<Card.Footer>
						<Button
							variant="secondary"
							class="w-full"
							onclick={() => verDetalle(hotel._id || hotel.id)}
						>
							{$t('hotels.detail')}
						</Button>
					</Card.Footer>
